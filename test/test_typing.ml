...
module VTypeTag = struct
  type t = vtype
end

module Vtype_ast_qcheck_testing = Ast.QCheck_testing (VTypeTag) (VTypeTag)
...
let test_cases_arb_compound_expr_typing : test list =
  let open QCheck in
  let open QCheck.Gen in
  let expr_gen ~(type_ctx : TestingTypeCtx.t) (t : vtype) : plain_expr Gen.t =
