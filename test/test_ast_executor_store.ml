...
let int_value_arb =
  let open QCheck in
  map (fun x -> SimpleExecutor.Int x) int

let bool_value_arb =
  let open QCheck in
  map (fun x -> SimpleExecutor.Bool x) bool
...
let non_empty_store_arb =
  ...
  map
    (fun kvs ->
      List.fold_left kvs
        ~f:(fun store (key, value) ->
          SimpleExecutor.store_set store ~key ~value)
        ~init:SimpleExecutor.empty_store)
    store_gen
...
let test_store_get_from_empty =
  ...
  Test.make ~count:100 ~name:"store_get_from_empty" key_arb (fun key ->
      let store = SimpleExecutor.empty_store in
      let res = SimpleExecutor.store_get store key in
      equal_option SimpleExecutor.equal_value res None)
...
let test_store_empty_set_then_get =
  ...
  Test.make ~count:100 ~name:"store_empty_set_then_get" (pair key_arb value_arb)
    (fun (key, value) ->
      let store = SimpleExecutor.empty_store in
      let store' = SimpleExecutor.store_set store ~key ~value in
      let res = SimpleExecutor.store_get store' key in
      equal_option SimpleExecutor.equal_value res (Some value))
...
let test_store_populated_overwrite =
  ...
  Test.make ~count:100 ~name:"store_populated_overwrite"
    ...
    (fun (store, key, old_value, new_value) ->
      let store' = SimpleExecutor.store_set store ~key ~value:old_value in
      let store'' = SimpleExecutor.store_set store' ~key ~value:new_value in
      let res = SimpleExecutor.store_get store'' key in
      equal_option SimpleExecutor.equal_value res (Some new_value))
