...
let test_cases_to_source_code_inv =
  ...
  Test.make ~count:1000 ~name:"Program to source code"
    (program_arb PrintExprSource Gen.unit) (fun prog ->
      match run_frontend_string (Program.program_to_source_code prog) with
      | Ok prog_out ->
          if Program.equal_program equal_unit prog prog_out then true
          else
            Test.fail_reportf
              "Got different Program. Expected:\n\n%s\n\nActual:\n\n%s"
              (get_program_printer PrintExprSource prog)
              (get_program_printer PrintExprSource prog_out)
      | Error err ->
