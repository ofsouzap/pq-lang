    ...
    let rec type_pattern
      ...
      | PatConstructor (v, c_name, p) -> (
          ...
          | Some ((vt_name, _), (_, c_t)) ->
              ...
              TypeCtx.compatible_types type_ctx ~exp:p_t ~actual:c_t
              ...
              if types_compatible then
                ...
                Error
                  (PatternTypeMismatch (pattern_to_plain_pattern p, p_t, c_t)))
      ...
      List.map
        ~f:(function
          ...
          | QuotientType qt ->
              List.map
                ~f:
                  (type_eqcons ~type_ctx ~quotient_type_name:qt.name
                     ~base_type_name:qt.base_type_name)
                qt.eqconss
              |> Result.all
              >>| fun eqconss ->
